...
module.exports = {
	...
	Query: {
		...
		async getFollower(parent, { userId, followerId }, context, info) {
			if(!userId || !followerId) throw new Error('Empty fields')
			else if((userId && userId.toString().trim() === '') || (followerId && followerId.toString().trim() === '')) throw new Error('Empty fields')

			try {
				const user = await User.findById(userId).populate('followers')
				const follower = user.followers.find(flwr => flwr.get('id') === followerId)
		...
		async searchUser(parent, { username }, context, info) {
			if(!username) return []
